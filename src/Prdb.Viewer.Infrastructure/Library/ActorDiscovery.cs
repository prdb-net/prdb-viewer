...
public sealed class ActorDiscovery(ViewerDbContext database, LibraryDiscovery library)
{
        ...
        return new ActorIndexPage(
            page.Take(take)
                .Select(row => new ActorSummary(
                    ...
                    ActorFacts.Stated(row.GenderLabel),
                    row.Videos,
                    row.ProfileState,
                    row.Favourite))
                .ToArray(),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new ActorDetail(
            ...
            // Read through the same rule the fetch writes by: a profile fetched before that rule
            // existed has the word "Unknown" stored, and it is no more a fact now than it was then.
            ...
            actor.Birthplace,
            ActorFacts.Stated(actor.HaircolourLabel),
            ActorFacts.Stated(actor.EyecolourLabel),

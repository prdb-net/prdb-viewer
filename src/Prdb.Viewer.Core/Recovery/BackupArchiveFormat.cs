...
public static class BackupArchiveFormat
{
    ...
    ///
    /// Version 3 replaces the one-to-five Personal Rating with a Personal Reaction. A version 1 or
    /// 2 archive still restores, and the ratings it carries are discarded rather than mapped, per
    /// ADR 0022. That is why the reader has to know the retired name at all: it refuses members it
    /// does not recognise, so a field that is deliberately dropped has to be dropped deliberately.
    /// </remarks>
